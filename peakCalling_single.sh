...
if [[ ${one} == *Yerkes* ]]; then
	...
	if [ -r $CC_SCORES_FILE ]; then
		...
		rm ${dirres}/${ID}_Yerkes/macs2_broad_ctrl_fil_dn_lam/* -rf
	fi
        run_spp.R -c=${inputBam}.sor.bam.fil.bam -p=8 -filtchr=chrM -savp=${CC_PLOT_FILE} -out=${CC_SCORES_FILE} -x=${EXCLUSION_RANGE_MIN}:${EXCLUSION_RANGE_MAX}
	conda deactivate
        export PATH=$rawPath
	...
	mkdir -p ${dirres}/${ID}_Yerkes/macs2_broad_ctrl_fil_dn_lam
	...
	rm -rf ${inputBam}.sor.bam.fil.bam_bed.gz

	module load macs/2.2.7.1
	mkdir -p ${dirres}/${ID}_Admera/macs2_broad_ctrl_fil_dn_lam
	macs2 callpeak -t ${inputBam}.sor.bam.fil.bam -c $ctrlBam \
	--scale-to small \
	--nomodel  \
	--broad \
	...
	module load macs/2.2.7.1
	mkdir -p ${dirres}/${ID}_Novogene/macs2_broad_ctrl_fil_dn_lam
	macs2 callpeak -t ${inputBam}.sor.bam.fil.bam -c $ctrlBam \
	--scale-to small \
	--nomodel  \
	--broad \
